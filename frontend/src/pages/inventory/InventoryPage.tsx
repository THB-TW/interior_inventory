...
export default function InventoryPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: WarehouseInventoryRequest }) =>
  ...
  const allocateMutation = useMutation({
    mutationFn: ({ inventoryId, projectId }: { inventoryId: number; projectId: number }) =>
      inventoryService.allocateToProject(inventoryId, projectId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory'] });
      ...
      <main className="flex-1 p-6 overflow-auto">
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <select
              value={statusFilter}
              onChange={(e) => { setStatusFilter(e.target.value as WarehouseStatus | ''); setCurrentPage(0); }}
            ...
            >
              ...
              {Object.values(WarehouseStatus).map((status) => (
                <option key={status} value={status}>
                  {WarehouseStatusConfig[status as WarehouseStatus].label}
                </option>
              ))}
            </select>
          </div>
                ...
                <tbody className="text-sm text-slate-700 divide-y divide-slate-100">
                  {pagedInventory.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border ${WarehouseStatusConfig[item.status].color}`}
                        >
